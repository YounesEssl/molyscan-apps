...
export class PimSyncService {
  ...
  async status() {
    const [activeIndex, latestRun, recentRuns, counts] = await Promise.all([
      ...
      Promise.all([
        this.prisma.pimProduct.count({ where: { active: true } }),
        this.prisma.pimReference.count({ where: { active: true } }),
        this.prisma.pimProduct.count({ where: { active: true, productType: 'lubricant' } }),
      ]),
    ]);
    return { activeIndex, latestRun, recentRuns, productCount: counts[0], referenceCount: counts[1], assistantProductCount: counts[2], running: this.running };
  }
