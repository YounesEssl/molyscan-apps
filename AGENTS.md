...
Guidance pour Codex quand il travaille dans ce repo.
...
| `chat/rag` | Pipeline RAG : reformulation Gemini → Supabase vector search → Codex |
...
Supabase RPC search_chunks — dual search (match_count=40), filtre équipements
  ↓
Codex Sonnet 4.6 — sélection et réponse (system prompt avec règles métier)
```
